...
registerAction2(class extends Action2 {
	...
	public run(
		...
		if (isNativeAgentSessionsUIEnabled(product) && !walkthroughID && !chatEntitlementService.sentiment.hidden && configurationService.getValue<string>('workbench.startupEditor') === 'agentSessionsWelcomePage') {
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'workbench.startupEditor': {
			...
			'type': 'string',
			'enum': ['none', 'welcomePage', 'readme', 'newUntitledFile', 'welcomePageInEmptyWorkbench', 'terminal', ...(isNativeAgentSessionsUIEnabled(product) ? ['agentSessionsWelcomePage'] : [])],
			'enumDescriptions': [
				...
				...(isNativeAgentSessionsUIEnabled(product) ? [localize({ comment: ['This is the description for a setting. Values surrounded by single quotes are not to be translated.'], key: 'workbench.startupEditor.agentSessionsWelcomePage' }, "Open the Agent Sessions Welcome page. Will override the workbench secondary side bar visibility settings.")] : []),
			],
			'default': 'welcomePage',
