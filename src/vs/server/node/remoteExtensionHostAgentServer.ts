	...
	private async _handleConnectionType(remoteAddress: string, _logPrefix: string, protocol: PersistentProtocol, socket: NodeSocket | WebSocketNodeSocket, isReconnection: boolean, reconnectionToken: string, msg: ConnectionTypeRequest): Promise<void> {
		...
		if (msg.desiredConnectionType === ConnectionType.Management) {
			...
			if (isReconnection) {
				...
				const dataChunk = protocol.readEntireBuffer();
				protocol.dispose();
				const con = this._instantiationService.createInstance(ExtensionHostConnection, reconnectionToken, remoteAddress, socket, dataChunk, undefined);
...
`;
	...
	if (hasWebClient && address && typeof address !== 'string') {
		...
		const protocol = args['tls-key-path'] ? 'https' : 'http';
		const defaultPort = protocol === 'https' ? 443 : 80;
		console.log(`Web UI available at ${protocol}://localhost${address.port === defaultPort ? '' : `:${address.port}`}${serverBasePath ?? ''}${queryPart}`);
	}
