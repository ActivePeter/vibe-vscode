...
export class VibeAuthenticationContribution extends Disposable {
	...
	protected get uiLanguage(): string { return language; }

	protected requestResource(url: string, signal: AbortSignal): Promise<Response> {
	...
	private async initialize(authPath: string): Promise<void> {
		...
		try {
			const response = await this.requestResource(`${authPath}/api/status`, controller.signal);
			if (!response.ok) {
				return;
			}
			...
			const title = localize2('signOut', "Sign Out");
			if (title.value === title.original && /^zh(?:-cn|-hans)?$/i.test(this.uiLanguage)) {
				// Upstream language packs do not yet contain this downstream contribution.
				try {
					const translation = await this.requestResource(FileAccess.asBrowserUri('vs/workbench/contrib/vibeAuthentication/browser/vibeAuthentication.nls.zh-cn.json').toString(true), controller.signal);
					if (translation.ok) {
						const messages: { signOut?: string } | null = await translation.json();
						if (typeof messages?.signOut === 'string' && messages.signOut) {
							title.value = messages.signOut;
						}
					}
				} catch { /* Keep the standard NLS fallback if the optional translation is unavailable. */ }
			}
...
registerWorkbenchContribution2(VibeAuthenticationContribution.ID, VibeAuthenticationContribution, WorkbenchPhase.AfterRestored);
