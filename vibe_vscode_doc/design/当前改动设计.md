...
共享 catalog 与快照写入隐藏的 Workspace 配置 `vibe.logicalWorkspaceState`。`LogicalWorkspaceStateStore` 监听配置变化，使同一个 Physical Workspace 的其他页面可以接收更新。

共享冲突采用整包 **last-write-wins**，不做字段级或 Workspace 级合并。这样删除和 owner 迁移具有明确语义，也符合当前阶段的复杂度选择。写队列暴露最新待写快照，防止较早一次本地写入产生的配置事件把内存状态短暂回滚。
...
- 所有共享数据在进入运行时前仍校验 Workspace ID、全局唯一资源归属和布局字段。
...
MainThread 的 handle record 同时保存 Webview input 与 presentation identity。Fullscreen panel 的后续 `reveal()` 始终路由到 `MODAL_GROUP`，不会逃入普通 editor group；record 随 input 生命周期统一清理。校验通过的首次打开使用 `{ fullscreen: true }`，覆盖 Workbench 且不显示普通 modal 边框。
...
- Logical Workspace：schema 迁移、页面 active ID、外部 LWW 更新、activation 顺序、初始投影和快速连续切换；
- 布局：active composite、显隐、尺寸和隐藏 part 尺寸恢复；
- 终端：前后台迁移、显式关闭、Workbench shutdown、本地与远程 persistent attach；
- Agent Session：快速跨 Workspace delta、全局 provider 集合、provider 删除、唯一 owner 和当前列表过滤；
- Fullscreen host：创建拒绝清理、dispose、reveal、未授权扩展和 modal 冲突；
