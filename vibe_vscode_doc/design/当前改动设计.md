...
channel 使用显式 `request/state` 协议，而不是只在写入时发送瞬时通知。页面完成监听注册后立即发送 `request`；已有页面广播当前 revision winner，使在先前广播之后、IndexedDB 延迟落盘之前启动的页面也能恢复最新快照。页面拒绝较旧的 `state` 时会把自己的 winner 定向回传给发送页，因此旧页面即使先用陈旧整包状态发起写入，也会重新收敛，而不会只在其他页面被拒绝后保留自己的分叉 cache。
...
- `createTerminal()` 入口在任何 profile、backend 或 CWD 异步解析前捕获 initiating Workspace ID，最终创建时不得重新读取可能已经切换的 active ID；
- contributed profile 使用必需的 `ITerminalCreationContext`，把 Workspace ID 与预分配的 Logical Terminal ID 作为一个不可拆散的值传过 direct profile、Agent Host、MainThread → ExtHost provider → MainThread 重入链路；普通 shell 与 extension PTY 都只能继承发起上下文，不能在 provider 返回时重新捕获 active Workspace；
- Logical Terminal ID 与 `forcePersist` 在启动进程前准备，并取得页面内 creation lease；projection 查询能看到 lease，因而同步的 instance-created 事件不会把新终端误移到后台，但 shared owner 只在 Terminal instance 成功创建后提交。location、split 或 group 创建失败会 dispose lease，不会产生没有实例可以清理的幽灵 ID；同一 ID 的并发 claim 只允许第一个成功创建的实例提交 owner，已持久化的 owner 始终优先，发起 Workspace 若被外部快照删除则回退到本页当前 Workspace；
- Terminal projection 在一个 feedback-coalescing transaction 中迁移整批前后台实例。自身触发的 `onDidChangeInstances` 只形成一次尾随 reconcile，不会使当前 generation 每迁移一个实例就失效；ownership 监听只选择 `id + terminalIds` slice，布局或 Session 写入不会启动终端扫描；
- 本地与远程重连都恢复 layout 中的 `tabs` 和 `background`。非当前 Workspace 的远程 PTY 刷新后仍会先恢复为后台 Terminal instance，之后可由 adapter 在切回时重新挂入前台。
...
- provider delta 通过 `updateChatSessionOwnership()` 在 Service 内执行一次 immutable state transition：先释放 `removed`，再认领 `addedOrUpdated`，且只广播一个 committed snapshot；因此 A 中 remove 后 500ms 内在 B 中复用同一 URI 时，新 owner 不会被旧的 first-owner-wins 记录阻挡；
- throttled resolver 同时携带触发时捕获的 Workspace ID；即使 500ms 内多个请求被合并，first-owner-wins 也不会把早先会话改绑到后来切入的 Workspace；
- provider resolve 前后按该 provider 的资源差集计算真实删除，并与完整 provider catalog 一起交给同一个 ownership transaction 清理 owner；
- 释放 live Chat model 不等于删除 provider item。`LocalAgentsSessionsController` 会在 model dispose 后从已经持久化的 live/history authority 重新 reconcile；只有权威目录确实缺少该 URI 时才发出 `removed` 并解除 owner；
- model-to-item 的异步转换在提交前校验当前 live model identity，并用 per-resource generation 约束可能等待 editing diff 的 stats 计算。已被 dispose、替换或被更新调用超越的 model 结果直接丢弃，不能在历史 reconcile 之后补发过期 `removed` 或覆盖新状态；
...
- ownership slice 本身就是 post-commit 通知，不再由 Model 的局部 depth guard 猜测何时补发事件。Model 还会比较真实的可见 Session resource projection：只有 owner 变化确实改变当前列表时才立即广播，避免 added owner 先于异步 catalog 到达而让观察者过早读取空列表；完整 resolve 则先提交 `_sessions` 和派生状态，再提交 owner transaction，并用单调 change sequence 保证无论该 transaction 是否改变当前 slice 都至少发布一次 catalog 提交事件。
- Local provider 的每次 refresh 都带单调 generation。只有最新 generation 在 live 与 history 均成功读取、最后一个异步边界后仍未取消时，才能原子替换 `_items` 并计算 delta；失败、取消和过期结果保留上一个完整 catalog，绝不会把 partial/empty fallback 当作权威删除。新 model 首次 refresh 尚未安装增量 listener 的窗口还会在 commit 前重新检查 `hasRequests`，请求已被删除的旧 live item 不会短暂闪入目录。
...
4. Explorer 的 `roots` 与 `findClosest*` 始终表示窗口级完整 model authority；Project 只改变明确命名的 `visibleRoots` tree projection，隐藏 root 的编辑器、decorations 和 file actions 仍能查询全局模型；
...
创建 RPC 现在返回 `Promise<void>`，并等待 `IEditorService.openEditor()` 真正完成，而不是只等待 Webview input 的同步构造。editor open 异步失败时，Workbench 会先 dispose 尚未取得 owner 的 input，再把 rejection 传回 ExtHost；ExtHost 随后 dispose panel/webview、删除 handle map，并触发正常的 `onDidDispose`，扩展不会留下永久阻塞后续打开的幽灵引用。

Extension API 的 `createWebviewPanel()` 仍是同步返回。ExtHost 因此为 pending handle 建立共享 operation queue：`html`、options、message、title、icon、reveal 和 dispose 按调用顺序等待创建 RPC 成功后再发送；创建失败则由统一 rejection lifecycle 清理，队列不会向不存在的 MainThread handle 投递操作。MainThread 完成 handle 注册后会主动 reconcile 一次 view state，补回 editor open 期间已经发生的 active/visible 事件。

Workbench 内部 `openWebview()` 已变为异步 owner acquisition。需要保持 singleton 的调用方不能等 Promise 完成后才发布 identity；`ReleaseNotesManager` 保存共享 pending creation Promise，并让所有并发 `show()` 复用同一个 Webview input，创建失败后再清空 pending 状态。
...
- 扩展自身遵循其他 browser extension 的统一脚本：`compile-web = bundle-web + typecheck-web`，`watch-web` 同时运行 bundle 与 typecheck watcher；默认开发构建不会只生成可运行 bundle 却漏报 TypeScript/proposed API 漂移。
- browser-only 扩展同时提供标准 `esbuild.mts` 桌面产品打包入口，并复用同一 browser bundle；`compile-non-native-extensions-build` 从干净 checkout 也会生成 `.build/extensions/dever-project-switcher/dist/browser/extension.js`。
- 开发 Web server 按规范化 locale 依次查找 `package.nls.<locale>.json`、父语言和默认 bundle，再复用 `localizeManifest` 以 localized/default 两层翻译清单生成 manifest；Web-first 开发入口与产品扫描不再分别得到中文和英文元数据。
...
- Logical Workspace：内部 Workspace storage 与外部 change、启动 request/announce、较旧 writer 的 winner 回传、schema 迁移、畸形 `unknown` 输入、页面 active ID、外部 LWW 更新、统一 state slice、activation 顺序、初始投影和快速连续切换；
- 布局：active composite、显隐、尺寸和隐藏 part 尺寸恢复；
- 终端：多实例前后台 transaction、创建失败不提交 owner、同 ID 并发 claim、创建期间切换 Workspace、direct/Agent Host/contributed provider 挂起后的上下文继承、显式关闭、Workbench shutdown、本地与远程 foreground/background persistent attach；
- Agent Session：逆序 refresh generation、history 失败、取消后的 partial snapshot、首次 refresh 资格重检、快速跨 Workspace delta、remove→re-add URI 原子迁移、provider resolve 失败后的 post-commit 刷新、owner/catalog 事件顺序、live model unload 后保留持久 owner、真实删除、过期异步 model result、全局 provider 集合、唯一 owner、同 active ID 的外部 ownership 更新和布局无关变更抑制；
- Fullscreen host：等待 editor open、pending handle 操作顺序、首屏 HTML、异步创建拒绝清理、立即 dispose、真实 `WebviewPanel.reveal()` options 透传、未授权扩展和 modal 冲突；
- Sessions entry：Web 启动前确认 `ILogicalWorkspaceService` 已进入 singleton registry；
- Project Context：Quick Pick active item、新增 root、全局 Explorer model 与 visible tree root 边界、嵌套 Git repository、延迟 SCM 注册和 selection 竞态；
