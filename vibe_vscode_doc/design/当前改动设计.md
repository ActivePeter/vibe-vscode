...
- contributed profile 把该 ID 作为内部、不可变的 terminal creation context 传过 MainThread → ExtHost provider → MainThread 重入链路；普通 shell 与 extension PTY 都只能继承发起上下文，不能在 provider 返回时重新捕获 active Workspace；
...
- 同一 delta transaction 先解除 `removed` URI 的旧 owner，再处理 `addedOrUpdated`；因此 A 中 remove 后 500ms 内在 B 中复用同一 URI 时，新 owner 不会被旧的 first-owner-wins 记录阻挡；
...
- model-to-item 的异步转换在提交前校验当前 live model identity。已被 dispose 或替换的 model 结果直接丢弃，不能在历史 reconcile 之后补发过期 `removed`；
...
- 终端：前后台迁移、创建期间切换 Workspace、contributed provider 挂起后的 ExtHost 重入、显式关闭、Workbench shutdown、本地与远程 foreground/background persistent attach；
- Agent Session：快速跨 Workspace delta、remove→re-add URI 迁移、live model unload 后保留持久 owner、真实删除、过期异步 model result、全局 provider 集合、唯一 owner、同 active ID 的外部 ownership 更新和布局无关变更抑制；
