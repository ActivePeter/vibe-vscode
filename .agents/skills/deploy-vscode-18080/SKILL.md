...
- stops the canonical `dever_vscode_latest` tmux service when it exists;
- invokes the Dever-side control entry, which builds only `/mnt/ceph/vibe-vscode`;
- preserves the existing state under `/mnt/ceph/dever_for_dev/.dever/vscode-services/state/latest`;
- waits for an HTTPS `200` response on port `18080` before succeeding;
- fails instead of killing an unrecognized process when the port is not owned by the canonical tmux session.

On failure, report the relevant tail from `latest.log` and leave the error visible. Do not fall back to `/mnt/ceph/dever_for_dev/third_party/vscode` as source, do not start an ad-hoc server, and do not touch port `18081` unless the user explicitly expands the deployment scope.
