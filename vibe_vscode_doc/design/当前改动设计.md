...
- 终端：前后台迁移、创建期间切换 Workspace、显式关闭、Workbench shutdown、本地与远程 foreground/background persistent attach；
- Agent Session：快速跨 Workspace delta、remove→re-add URI 迁移、全局 provider 集合、provider 删除、唯一 owner、同 active ID 的外部 ownership 更新和布局无关变更抑制；
- Fullscreen host：等待 editor open、异步创建拒绝清理、dispose、真实 `WebviewPanel.reveal()` options 透传、未授权扩展和 modal 冲突；
